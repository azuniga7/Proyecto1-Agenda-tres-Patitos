...
try:
    ...
    class _MissingCustomTkinter:
        def __getattr__(self, name):
            raise ImportError(
                "customtkinter is required to run this app. "
...
class AppAgenda(ctk.CTk):
    ...
    def crear_sidebar(self):
        ...
        ##Se ajustaron los valores para que se viera el boton de ubicaciones ya que recargar datos lo tapaba
    ...
    def datos_evento_formulario(self):
        ...
        ubicacion_valor = self.combo_ev_ubicacion.get()
        ubicacion = None if ubicacion_valor == "Sin ubicación" else self.ubicaciones_combo.get(ubicacion_valor)
        try:
    ...
    def cargar_datos_eventos(self):
        try:
            ...
            ubicaciones_rows = self.ejecutar_consulta("SELECT id_ubicacion, nombre FROM ubicaciones ORDER BY nombre", fetch=True)
            self.ubicaciones_combo = {f"{nombre} — #{uid}": uid for uid, nombre in ubicaciones_rows}
    ...
    def configurar_pestana_ubicaciones(self):
        self.crear_encabezado(self.tab_ubicaciones, "Ubicaciones", "Administra los recintos y espacios físicos donde se realizan los eventos.")
    ...
    def actualizar_ubicacion(self):
        ...
        try:
            ...
            self.cargar_datos_ubicaciones(); messagebox.showinfo("Éxito", "Ubicación actualizada.")
        except Exception as e:
            messagebox.showerror("Error", str(e))
    ...
    def configurar_pestana_tareas(self):
        ...
        self.tree_tareas = self.crear_treeview(
            tabla_frame, ("ID", "Evento", "Responsable", "Título", "Prioridad", "Fecha límite", "Estado"),
            (50, 150, 150, 160, 90, 110, 100)
        )
        ...
        fila_fecha = ctk.CTkFrame(form, fg_color="transparent"); fila_fecha.pack(fill="x", padx=10)
        self.fecha_limite_tarea = self.crear_selector_fecha(fila_fecha)
        self.fecha_limite_tarea.pack(side="left", fill="x", expand=True)
    ...
    def cargar_tarea_seleccionada(self, _=None):
        ...
        try:
            fecha = datetime.strptime(str(vals[5]), "%Y-%m-%d")
            self.establecer_fecha(self.fecha_limite_tarea, fecha)
        except ValueError:
            pass
    ...
    def datos_tarea_formulario(self):
        ...
        return titulo, descripcion, prioridad, fecha_limite, estado, evento, responsable
    ...
    def agregar_tarea(self):
        try:
            ...
            self.ejecutar_consulta("""
                INSERT INTO tareas
                (titulo, descripcion, prioridad, fecha_limite, estado, id_evento, id_usuario_responsable)
    ...
    def actualizar_tarea(self):
        ...
        try:
            titulo, descripcion, prioridad, fecha_limite, estado, evento, responsable = self.datos_tarea_formulario()
            self.ejecutar_consulta("""
                UPDATE tareas SET titulo=%s, descripcion=%s, prioridad=%s, fecha_limite=%s,
                estado=%s, id_evento=%s, id_usuario_responsable=%s WHERE id_tarea=%s
            """, (titulo, descripcion, prioridad, fecha_limite, estado, evento, responsable, tid))
    ...
    def cargar_datos_tareas(self):
        try:
            ...
            for row in rows:
                evento_label = f"{row[2]} — #{row[1]}"
                responsable_label = f"{row[4]} {row[5]} — #{row[3]}"
                fecha = row[8].strftime("%Y-%m-%d") if hasattr(row[8], "strftime") else row[8]
                self.tree_tareas.insert("", "end", values=(row[0], evento_label, responsable_label, row[6], row[7], fecha, row[9]))


            eventos_rows = self.ejecutar_consulta("SELECT id_evento, titulo FROM eventos ORDER BY titulo", fetch=True)
            self.eventos_combo = {f"{titulo} — #{eid}": eid for eid, titulo in eventos_rows}
            valores_evento = ["Seleccione un evento"] + list(self.eventos_combo.keys())
            self.combo_tarea_evento.configure(values=valores_evento)


            valores_resp = ["Seleccione un usuario"] + list(self.usuarios_combo.keys())
    ...
    def configurar_pestana_disponibilidad(self):
        self.crear_encabezado(self.tab_disponibilidad, "Disponibilidad", "Administra las franjas de disponibilidad de cada usuario.")
        ...
        self.tree_disponibilidad = self.crear_treeview(
            tabla_frame, ("ID", "Usuario", "Tipo", "Fecha", "Hora inicio", "Hora fin"),
            (50, 160, 110, 100, 100, 100)
        )
        ...
        fila_fecha = ctk.CTkFrame(form, fg_color="transparent"); fila_fecha.pack(fill="x", padx=10)
        self.fecha_disp = self.crear_selector_fecha(fila_fecha)
        self.fecha_disp.pack(side="left", fill="x", expand=True)
    ...
    def cargar_disponibilidad_seleccionada(self, _=None):
        ...
        try:
            ...
            pass
        self.entry_disp_hora_inicio.delete(0, tk.END); self.entry_disp_hora_inicio.insert(0, str(vals[4])[:5])
        self.entry_disp_hora_fin.delete(0, tk.END); self.entry_disp_hora_fin.insert(0, str(vals[5])[:5])
    ...
    def datos_disponibilidad_formulario(self):
        ...
        hora_inicio_txt = self.entry_disp_hora_inicio.get().strip()
        hora_fin_txt = self.entry_disp_hora_fin.get().strip()
        if usuario is None or tipo is None or not hora_inicio_txt or not hora_fin_txt:
            raise ValueError("Completa usuario, tipo, hora de inicio y hora de fin.")
        try:
            hora_inicio = datetime.strptime(hora_inicio_txt, "%H:%M").time()
            hora_fin = datetime.strptime(hora_fin_txt, "%H:%M").time()
        except ValueError:
            raise ValueError("Las horas deben tener formato HH:MM, ej. 09:00")
    ...
    def actualizar_disponibilidad(self):
        ...
        try:
            ...
            self.cargar_datos_disponibilidad(); messagebox.showinfo("Éxito", "Disponibilidad actualizada.")
        except Exception as e:
            messagebox.showerror("Error", str(e))
    ...
    def eliminar_disponibilidad(self):
        did = self.disponibilidad_seleccionada_id()
        if did is None: return messagebox.showwarning("Selección requerida", "Selecciona una franja.")
        if not messagebox.askyesno("Confirmar", "¿Eliminar la disponibilidad seleccionada?"): return
        try:
        ...
        try:
            rows = self.ejecutar_consulta("""
                SELECT d.id_disponibilidad, u.id_usuario, u.nombre, u.apellido,
                       t.id_tipo, t.nombre, d.fecha, d.hora_inicio, d.hora_fin
            ...
            for row in rows:
                usuario_label = f"{row[2]} {row[3]} — #{row[1]}"
                fecha = row[6].strftime("%Y-%m-%d") if hasattr(row[6], "strftime") else row[6]
                self.tree_disponibilidad.insert("", "end", values=(row[0], usuario_label, row[5], fecha, row[7], row[8]))


            tipos_rows = self.ejecutar_consulta("SELECT id_tipo, nombre FROM tipos_disponibilidad ORDER BY id_tipo", fetch=True)
            self.tipos_disponibilidad_combo = {nombre: tid for tid, nombre in tipos_rows}
            valores_tipo = ["Seleccione un tipo"] + list(self.tipos_disponibilidad_combo.keys())
            self.combo_disp_tipo.configure(values=valores_tipo)


            valores_usuario = ["Seleccione un usuario"] + list(self.usuarios_combo.keys())
            self.combo_disp_usuario.configure(values=valores_usuario)
        except Exception as e:
